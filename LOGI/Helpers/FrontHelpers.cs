...
namespace LOGI.Helpers
{
    public static class FrontHelpers
    {
        ...
        public static string QuotedDescription(this HtmlHelper helper, string description, string title, string url)
        {
            if (description == null)
                return "";

            string replacement = "<div class='quote__text quote'><span class='icon-open-quotations'></span><span class='quote__body'>[data-temp]$2[/data-temp]</span><span class='icon-closed-quotations'></span>";
            replacement += "<a class='twitter-button' target='_blank' href='https://twitter.com/intent/tweet?text=[data-temp]$2[/data-temp]+" + url + "' data-size='large'><span class='icon-twitter'></span>Tweet quote </a></div>";
            ...
            {
                result = result.Replace(m.Value, replacement.Replace("$2",m.Value.Replace("<blockquote>", "").Replace("</blockquote>", "")));
            }
            
            //string result =  Regex.Replace(description, @"(<blockquote>)(.*)(</blockquote>)", replacement, RegexOptions.Singleline);

            result = result.Replace("[data-temp]<p>", "");
            result = result.Replace("</p>[/data-temp]", "");

            result = result.Replace("[data-temp]\r\n<p>", "");
