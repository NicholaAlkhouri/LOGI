...
namespace LOGI.Services
{
    public class ImageService
    {
        ...
        public static bool SaveImage(Image image, string file_name, int width = 0, int height = 0, string directory_path = null)
        {
            ...
            {
                directory = HttpContext.Current.Server.MapPath("~" + directory_path);
            }
        ...
        public static Image ResizeImage(Image image, int width, int height)
        {
            ...
            try
            {
                ...
                using (Image img = image)
                {
                    thumbNail = new Bitmap(resize_width + 1, resize_height + 1, PixelFormat.Format32bppPArgb);    //create a bitmap for selected 
                ...
                {
                    thumbNail = new Bitmap(width, height, PixelFormat.Format32bppPArgb);    //create a bitmap for selected 
                ...
                //using (Image img = thumbNail)
                //{
                //    thumbNail = new Bitmap(width, height, PixelFormat.Format32bppPArgb);
                //    Graphics g = Graphics.FromImage(thumbNail);
                //    g.Clear(Color.White);
                //    g.CompositingQuality = CompositingQuality.HighQuality;
                //    g.SmoothingMode = SmoothingMode.AntiAlias;
